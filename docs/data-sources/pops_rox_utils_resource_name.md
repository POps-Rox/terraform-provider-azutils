...
page_title: "pops_rox_utils_resource_name Data Source - terraform-provider-pops-rox-utils"
subcategory: ""
description: |-
  Sample resource in the Terraform provider scaffolding.
---

# pops_rox_utils_resource_name (Resource)

The data source pops_rox_utils_resource_name generate a name for a resource (recommended way as data sources are evaluated before resources got created).
...
```hcl
data "pops_rox_utils_resource_name" "rg_example" {
  name          = "demogroup"
...
output "rg_example" {
  value = data.pops_rox_utils_resource_name.rg_example.result
}
```

```bash

data.pops_rox_utils_resource_name.rg_example: Reading...
data.pops_rox_utils_resource_name.rg_example: Read complete after 0s [id=anoa-eastus-demogroup-z-rg]
...
resource "azurerm_resource_group" "rg" {
  name     = data.pops_rox_utils_resource_name.rg_example.result
  location = "eastus"
}
```
```bash
