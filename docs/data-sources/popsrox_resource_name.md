...
The data source popsrox_resource_name generate a name for a resource (recommended way as data sources are evaluated before resources got created).
...
```hcl
data "popsrox_resource_name" "rg_example" {
  name          = "demogroup"
...
output "rg_example" {
  value = data.popsrox_resource_name.rg_example.result
}
```

```bash

data.popsrox_resource_name.rg_example: Reading...
data.popsrox_resource_name.rg_example: Read complete after 0s [id=anoa-eastus-demogroup-z-rg]
...
resource "azurerm_resource_group" "rg" {
  name     = data.popsrox_resource_name.rg_example.result
  location = "eastus"
}
```
```bash
