...
func getSlug(resourceType string, convention string) string {
	if convention == ConventionNoOps || convention == ConventionRandom {
		if val, ok := ResourceDefinitions[resourceType]; ok {
			return val.NoOpsPrefix
		}
	}
	return ""
}
...
func getResourceName(resourceTypeName string, separator string,
	...
	if !validationRegEx.MatchString(resourceName) {
		return "", fmt.Errorf("invalid name for naming %s %s, the pattern %s doesn't match %s", resource.ResourceTypeName, name, resource.ValidationRegExp, resourceName)
	}
